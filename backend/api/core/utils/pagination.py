...
class Paginator:
    ...
    def __init__(self, queryset: QuerySet, per_page: int = 15, base_url: str = "http://localhost", serializer_class=None):
        ...
        self.last_page = max(1, math.ceil(self.total / per_page))
    ...
    def paginate(self, page: int = 1) -> Dict[str, Any]:
        ...
        current_page = max(1, min(page, self.last_page))
        
        # Calcula offset e limit
        offset = (current_page - 1) * self.per_page
        
        # Obtém os dados para a página atual
        data = list(self.queryset.skip(offset).limit(self.per_page))
        ...
        pagination_data = {
            ...
            "last_page_url": f"{self.base_url}?page={self.last_page}",
            "next_page_url": f"{self.base_url}?page={current_page + 1}" if current_page < self.last_page else None,
            "prev_page_url": f"{self.base_url}?page={current_page - 1}" if current_page > 1 else None,
            "path": self.base_url,
            "from": from_index,
            "to": to_index,
            "data": [self._serialize_with_serializer(doc) for doc in data]
        }
